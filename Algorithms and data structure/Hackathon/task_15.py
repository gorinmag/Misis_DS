...
df1 = df[df['speed']>0]
print(f"Средняя скорость для каждого часа: {df1.groupby(['hourly'])['speed'].mean()}")
print(f"Максимальная скорость для каждого часа: {df1.groupby(['hourly'])['speed'].mean().max().round(2)}")